...
const Sidebar = (props: Props) => {
  ...
  if (size.width && size.width > 600) {
    ...
    return (
      <div className='absolute top-0  h-full w-full flex items-end'>
        <div className='flex flex-col justify-left bg-gray-200 w-full'>
          <div className='flex flex-row-reverse justify-around'>
            {props.routes.slice(9, 12).map(({ path, name }) => (
              //   I can add an icon for each route later
              <div key={name} className='py-2 flex justify-center'>
                <Link to={path}>{name}</Link>
              </div>
            ))}
          </div>
