...
const Navbar = (props: Props) => {
  ...
  if (props.size.width! > 600) {
    ...
    return (
      <div>
        <div className='flex flex-row justify-between self-center px-2 py-1 bg-gray-400 border-b-2 border-gray-800'>
          {/* Toggle Sidebar */}
          <div className='flex self-center'>
            <div className='flex self-center font-bold lg:text-xl text-sm px-2'>
              {/* can replace this with an icon later */}
              <Link to='/'>
                <h1>EbutUoy</h1>
              </Link>
          ...
          <div className='flex self-center'>
            {props.isAuthenticated ? (
              ...
              <Link
                to='/authenticate'
                className='px-2 py-1 border-purple-600 border-2 text-purple-600 font-semibold text-center'>
                {/*  can add a user icon here later */}
                Sign In
              </Link>
            )}
